...
/**
 * Utility page object for navigating to pages with socket extensions
 *
 * Uses menu-based navigation to ensure reliability when URLs change.
 *
 * Supports testing Foundry extensions that appear in various sockets:
 ...
 * - hosts.host.panel (Host Management)
 */
export class SocketNavigationPage extends BasePage {
  ...
  /**
   * Navigate to Host Management page (hosts.host.panel socket)
   * Uses menu navigation: Menu → Endpoint security → Host management
   */
  async navigateToHostManagement(): Promise<void> {
    return this.withTiming(
      async () => {
        ...
        // Click "Host management" link
        const hostManagementLink = this.page.getByRole('link', { name: /Host management/i });
        await hostManagementLink.click();

        // Wait for page to load
        await this.page.waitForLoadState('networkidle');

        // Verify we're on the host management page
        const pageTitle = this.page.locator('h1, [role="heading"]').first();
        await expect(pageTitle).toBeVisible({ timeout: 10000 });
  ...
  /** Open first host to show details panel with extensions */
  async openFirstHost(): Promise<void> {
    return this.withTiming(
      async () => {
        await this.page.waitForLoadState('networkidle');
        ...
        // Click on the first hostname link in the table
        const firstHostLink = this.page.locator('table tbody tr:first-child td:first-child a, table tbody tr:first-child button').first();
        await firstHostLink.waitFor({ state: 'visible', timeout: 10000 });
        await firstHostLink.click();

        // Wait for the side panel to appear (Host Information section)
        await this.page.waitForSelector('text=/Host Information|Asset details/i', { state: 'visible', timeout: 15000 });
        await this.page.waitForLoadState('networkidle');

        this.logger.info('Opened first host details panel');
      },
      'Open first host'
    );
  }
